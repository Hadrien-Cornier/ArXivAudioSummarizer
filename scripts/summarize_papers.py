...
def summarize_papers(config: ConfigParser) -> None:
    ...
    csv_path: str = config.get('summarize_papers', 'csv_path', fallback='data/pdfs-to-summarize/papers_to_summarize.csv')
    ...
    for _ in range(3):
        try:
            stream = client.chat.completions.create(
                ...
                max_tokens=1500, n=1, stop=None, stream=True
            )
...
def generate_summary(paper: str, config: ConfigParser) -> str:
    ...
    for prompt in config.get('summarize_papers', 'prompts').split(','):
        ...
        all_messages.append({'role': 'assistant', 'content': chatbot(all_messages, config)})
    
    all_messages.append({'role': 'user', 'content': "Synthesize the above information into a concise summary of the paper's key contributions and significance."})
